...
def train_collate_1(batch):
    tf = transforms.Compose(
        [transforms.ToTensor(),
         transforms.Normalize([0.2926], [0.3003])
         ])
...
def train_collate_3(batch):
    tf = transforms.Compose(
        [transforms.RandomHorizontalFlip(0.3),
         ...
         transforms.RandomApply([transforms.RandomRotation(45)], 0.3),
         transforms.RandomApply([transforms.ColorJitter(0.5, 0.5, 0.5)], 0.3),
         transforms.ToTensor(),
         transforms.Normalize([0.2926], [0.3003])
         ])
    return train_collate(batch, tf)
...
def final_test_collate(batch):
    tf = transforms.Compose(
        [transforms.ToTensor(),
         transforms.Normalize([0.2926], [0.2996])
         ])
    transform_data = []
    for imgnp in batch:
        imgnp = imgnp.reshape(28, 28)
...
def train_model(modelclass, train_data, train_label, threshold):
    ...
    for epoch in range(num_epochs):
        ...
        if (newacc > test_acc and test_acc > 0.8 + train_stage*0.02) or epoch - last_stage_epoch > 7:
            train_stage += 1
...
def continue_train_model(modelnum, beginepoch, train_data, train_label, threshold, train_stage):
    model = Model()
    ...
    if train_stage == 2:
        ...
        new_train_collate = train_collate_1
    train_loader = Data.DataLoader(dataset=train_dataset, batch_size=BATCH_SIZE, shuffle=True,
                                   collate_fn=new_train_collate)
    test_loader = Data.DataLoader(dataset=test_dataset, batch_size=BATCH_SIZE, shuffle=False, collate_fn=test_collate)
    ...
    for epoch in range(beginepoch + 1, num_epochs):
        train_loss = 0  # 定义训练损失
        train_acc = 0  # 定义训练准确度
        model.train()  # 将网络转化为训练模式
        for i, (X, label) in enumerate(train_loader):  # 使用枚举函数遍历train_loader
        ...
        if (newacc > test_acc and newacc > 0.8) or epoch - last_stage_epoch > 20:
            train_stage += 1
            last_stage_epoch = epoch
            print("train stage changed: " + str(train_stage))
            # lr = lr / 2
...
def save_model(model, modelnum, epoch):
    path_prefix = "./data/model"
    path_suffix = ".pt"
...
if __name__ == "__main__":
    ...
    model = train_model(Model, train_data, train_label, 0.95)
    # model = continue_train_model(3,26,train_data,train_label,0,4)
